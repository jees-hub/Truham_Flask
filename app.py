...
def signin():
    ...
    connection = pymysql.connect(user = 'root', host = 'localhost', password = '', database = 'truhamsokogarden')
    # defining the cursor, we are using the dictionary cursor
    cursor = connection.cursor(pymysql.cursors.DictCursor)
    # creating the sql query
    sql = "select * from users where email = %s and password = %s"
    # defining the data to replace the placeholder in the sql query
    data = (email, password)
    # ececute the sql query using the cursor
    cursor.execute(sql, data)
...
def add_products():
    ...
    # creating a connection between the database and flask 
    connection = pymysql.connect(user = 'root', host = 'localhost', password = '', database = 'truhamsokogarden')
    # defining the cursor
    cursor = connection.cursor()
    # creating the sql query 
    sql = "insert into product_details (product_name, product_description, product_cost, product_photo) values (%s,%s,%s,%s)"
    # the data that is to replace the placeholders
    data = (product_name,product_description,product_cost,filename)
    # executing the sql and the data
    cursor.execute(sql,data)
...
def get_product_detail():
    # estabilishing a connection to the database
    connection = pymysql.connect(user = "root", host = "localhost" , password = "" , database = "truhamsokogarden")
